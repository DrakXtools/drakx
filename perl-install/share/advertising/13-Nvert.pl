#!/usr/bin/perl

$width = 400;
$height = 270;
$icon = 0;
$icon_dx = 0;
$icon_dy = 0;
@data = (
	 [ _("For more information on MandrakeSoft's Professional Services and commercial offerings, please dial the toll-free number that you will find here:"), 0, 0, 400, 100, 0],
	 # This phone number has to be '1877 MDKCORP' for USA and Canada, '0821 231 231' for France. For other, I dunno! (dams)
	 [ _("http://www.mandrakesoft.com/sales/contact"), 0, 120, 400, 150 ],
#	 [ 'text', $x, $y, $area_width, $area_height ],
	);
